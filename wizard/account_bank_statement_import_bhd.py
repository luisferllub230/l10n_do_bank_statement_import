...
class AccountBankStatementImportBhd(models.TransientModel):
    ...
    def _parse_lines_csv(self, data_file, bank_entity, currency_code, currency_symbol):
        ...
        header_expected = ['Fecha', 'Referencia', 'Detalle de Transacciones', 'Débitos', 'Créditos', 'Balance']
        ...
        csv_options['doublequote'] = True

        new_data_file = self._normalise_doublequote_csv(data_file)
        
        csv = self._get_csv_iterator(new_data_file, csv_options)
        ...
        for i, body in enumerate(bodys):
            try:
                ...
                faild_lines.append({
                        'date': _date,
                        'payment_ref': body[reference_index],
                        'amount': _ammount,
